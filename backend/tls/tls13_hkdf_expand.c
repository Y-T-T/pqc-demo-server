...
uchar * sha256(const uchar *message, const size_t message_len) {
    uchar *md = malloc(SHA256_DIGEST_LENGTH * sizeof(uchar));
    unsigned int md_len;
...
uchar * sha384(const uchar *message, const size_t message_len) {
    uchar *md = malloc(SHA384_DIGEST_LENGTH * sizeof(uchar));
    unsigned int md_len;
...
uchar * hmac_sha384(const uchar *key, const size_t key_len, const uchar *data, const size_t data_len) {
    ...
    uchar *md = malloc(len * sizeof(uchar));
...
uchar * hkdf_extract(const uchar *salt, const size_t salt_len, const uchar *ikm,const size_t key_len){
    ...
    uchar *prk = malloc(EVP_MAX_MD_SIZE);
...
static uchar * hkdf_expand(const uchar *ss, const size_t ss_len, struct HkdfLabel hkdf_label, const uint16_t context_len, const uint16_t out_len){

    uchar *info = malloc(516 * sizeof(uchar));
    size_t info_len = 0;
    ...
    uchar *hexin, *md;
    uchar *res = malloc((out_len / (SHA384_DIGEST_LENGTH + 1) + 1) * SHA384_DIGEST_LENGTH * sizeof(uchar));
    ...
    while(res_len < out_len){
        ...
        hexin = concat_uc_str(hexin, res_len + info_len, (uchar *)&i, 1);
        // printf("hexin: ");
...
static uchar * hkdf_expand_label(const uchar *ss, const size_t ss_len, const char *label, const uchar *context, const uint16_t context_len, const uint16_t out_len){
    ...
    // print_bytes((uchar *)hkdf_label.label, strlen(hkdf_label.label));

    memcpy(hkdf_label.context, context, context_len);
    // printf("context: ");
    // print_bytes(hkdf_label.context, context_len);

    uchar *output = hkdf_expand(ss, ss_len, hkdf_label, context_len, out_len);
...
uchar * derive_secret(const uchar *ss, const size_t ss_len, const char *label, const uchar *msg,  const uint16_t context_len, const uint16_t out_len){
    // uchar *context = sha384(msg, context_len);

    uchar *output = hkdf_expand_label(ss, ss_len, label, msg, context_len, out_len);
...
//     uchar *hello_hash = malloc(hello_hash_length * sizeof(uchar));
//     size_t shared_secret_length = strlen(shared_secret_str) / 2;
//     uchar *shared_secret = malloc(shared_secret_length * sizeof(uchar));
...
//     uchar *derived_secret = derive_secret(early_secret, 48, "derived", empty_hash, 48, 48);
//     printf("ds: ");
//     print_bytes(derived_secret, 48);

//     uchar *handshake_secret = hkdf_extract(derived_secret, 48, shared_secret, shared_secret_length);
//     printf("hs: ");
//     print_bytes(handshake_secret, 48);

//     uchar *server_secret = derive_secret(handshake_secret, 48, "s hs traffic", hello_hash, 48, 48);
//     printf("ssec: ");
//     print_bytes(server_secret, 48);

//     uchar *client_secret = derive_secret(handshake_secret, 48, "c hs traffic", hello_hash, 48, 48);
//     printf("csec: ");
//     print_bytes(client_secret, 48);

//     uchar *server_handshake_key = derive_secret(server_secret, 48, "key", (uchar *)"", 0, 32);
//     printf("skey: ");
//     print_bytes(server_handshake_key, 32);

//     uchar *client_handshake_key = derive_secret(client_secret, 48, "key", (uchar *)"", 0, 32);
//     printf("ckey: ");
//     print_bytes(client_handshake_key, 32);

//     uchar *server_handshake_iv = derive_secret(server_secret, 48, "iv", (uchar *)"", 0, 12);
//     printf("siv: ");
//     print_bytes(server_handshake_iv, 12);

//     uchar *client_handshake_iv = derive_secret(client_secret, 48, "iv", (uchar *)"", 0, 12);
//     printf("civ: ");
